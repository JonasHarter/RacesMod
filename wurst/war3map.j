...
// Trigger: Cuff
//===========================================================================
library Cuff initializer initialize requires SimpleCuff, LeveledCuff

    globals
        // Length of a frame
        private constant real loopInterval = 0.03
        // The maximum number of iterations per frame
        private constant integer maxLoop = 4
    endglobals

    /*
        Cuff variants take a unit group and iterate over it
        Iterate returns true if the last one was reached
    */
    ...
    private function initialize takes nothing returns nothing
        ...
        call TriggerRegisterTimerEventPeriodic( trig, loopInterval )
        call TriggerAddAction( trig, function iterate )
    endfunction
...
library SimpleCuff
    ...
    globals
        private SimpleCuff array cuffList
        private integer listMax = 0
        // The current position in the list
        private integer listPointer = 0
    endglobals
    ...
    struct SimpleCuff
        ...
        static method create takes integer ab, integer buf returns thistype
            local thistype s = thistype.allocate()
            set s.cuffBuff = buf
            set s.cuffAbility = ab
            return s
        endmethod
    endstruct
    ...
    function iterateSimpleCuff takes group g returns boolean
        local unit u
        local SimpleCuff currentCuff
        set currentCuff = cuffList[listPointer]
        loop
            set u = FirstOfGroup(g)
            exitwhen u == null
            call GroupRemoveUnit(g, u)
            if( GetUnitAbilityLevel(u, currentCuff.cuffBuff) > 0 )then
                call UnitAddAbility(u, currentCuff.cuffAbility)
            else
                call UnitRemoveAbility(u, currentCuff.cuffAbility)
            endif
            set u = null
        endloop
        if (listPointer < listMax) then
            set listPointer = listPointer + 1
            return false
        endif
        set listPointer = 0
        return true
    ...
    public function createSimpleCuff takes integer ab, integer buf returns nothing
        local SimpleCuff sc = SimpleCuff.create(ab, buf)
        set cuffList[listMax] = sc
...
library LeveledCuff
    ...
    globals
        private LeveledCuff array cuffList
        private integer listMax = 0
        // The current position in the list
        private integer listPointer = 0
    endglobals
    ...
    struct LeveledCuff
        // 8192 / 3 = 2730
        integer array cuffBuff[3]
        integer array cuffAbility[3]

        static method create takes integer ab1, integer ab2, integer ab3, integer buf1, integer buf2, integer buf3 returns thistype
            local thistype s = thistype.allocate()
            set s.cuffBuff[0] = buf3
            set s.cuffBuff[1] = buf1
            set s.cuffBuff[2] = buf2
            set s.cuffAbility[0] = ab3
            set s.cuffAbility[1] = ab1
            set s.cuffAbility[2] = ab2
            return s
        endmethod
    endstruct
    ...
    public function createLeveledCuff takes integer ab1, integer ab2, integer ab3, integer buf1, integer buf2, integer buf3 returns nothing
        local LeveledCuff sc = LeveledCuff.create(ab1, ab2, ab3, buf1, buf2, buf3)
        set cuffList[listMax] = sc
        set listMax = listMax + 1
    endfunction
...
function InitCustomTriggers takes nothing returns nothing
    call InitTrig_Cuff(  )
    call InitTrig_SimpleCuff(  )
    call InitTrig_LevledCuff(  )
endfunction
